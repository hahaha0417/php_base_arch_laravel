...
trait accounts
{
    ...
    public function search(&$search, &$data_search_key, &$data_search, &$data, &$result)
    {
        ...
        {
            $parameter_temp->query_builder = $parameter_temp->query_builder->where(define_accounts::EMAIL, "like", "%{$data_search}%");
            // $search .= table_key::AND . " `" . define_accounts::EMAIL . "` like '%{$data_search}%' " ;
        }
        else if($data_search_key == table_key::GENDER)
        {
            $parameter_temp->query_builder = $parameter_temp->query_builder->where(define_accounts::GENDER, "=", "{$data_search}");
